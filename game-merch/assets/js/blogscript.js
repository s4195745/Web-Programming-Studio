document.addEventListener(
  ...
  async () => {
    ...
    // ---------------- track images during post creation/edit
    ...
    let CURRENT_USER = currentUsername || currentEmail || window.currentUserAccount || window.currentUser || '';
    let CURRENT_USER_ROLE = String( window.currentUserRole || sessionStorage.getItem('userRole') || '').trim().toLowerCase();
    ...
    if (!CURRENT_USER) {
      try {
        ...
        if (data.role) { CURRENT_USER_ROLE = String(data.role).trim().toLowerCase();
    ...
    function renderCommentsHtml(comments) {
      ...
      if (commentTemplate) {
        ...
        comments.forEach(comment => {
            ...
            if (author) { author.textContent = comment.author || 'Guest'; }
    ...
    async function submitComment(event, postId) {
      ...
      try {
        ...
        if (!postResponse.ok) {throw new Error('Failed to reload post');
        }
        ...
        alert(err.error ||'Error posting comment.'
        );
      }
    }

    // main blog feed
    if (postFeed) { fetchAndRenderFeed = function () {
          ...
          fetch( `/api/posts?${params.toString()}`)
            ...
            .then(
              posts => {postFeed.innerHTML = '';
                if (!Array.isArray(posts) ||
                  posts.length === 0) { postFeed.innerHTML = '<p class="no-posts">No blog posts found.</p>';
                  return;
                }
                ...
                posts.forEach(
                  post => {
                    ...
                    // catergories
                    if (categoryIcon) {
                      categoryIcon.textContent = post.categoryIcon || '📝';
                    }

                    if (categoryName) {categoryName.textContent = post.category ||'';
                    }
                    ...
                    // post summary
                    if (description) {description.textContent = post.summary ||'';
                    }

                    // date and author
                    if (meta) {meta.textContent = `${post.dateAdded || ''} • By ${post.author || 'Unknown'}`;
                    }
                    ...
                    // content
                    if (articleContent) {
                      articleContent.innerHTML = post.content? post.content.replace(/\n/g,'<br>'): '';
                    }
                    ...
                    if (commentList) {
                      ...
                      commentList.innerHTML = renderCommentsHtml(post.comments);
                    }
                    ...
                    if (readButton) {
                      ...
                      readButton.addEventListener('click',
                        () => {
                          ...
                          if (isHidden) { detail.style.display = 'block';
      ...
      // Search
      if (searchBtn) {
        searchBtn.addEventListener('click', fetchAndRenderFeed);
      }

      // Category
      if (categorySelect) {
        categorySelect.addEventListener('change', fetchAndRenderFeed);
      }
  ...
  () => {
    ...
    fetch('/api/posts?userOnly=true', {
      ...
      .catch(
        err => {
          ...
          userPostsList.innerHTML = '<p class="error-msg">Failed to load posts.</p>';
        }
      );
  };
      ...
      postForm.addEventListener('submit',
        event => {
          ...
          if (!summaryVal) {
            ...
            summaryVal = firstSentenceMatch? firstSentenceMatch[0]: (
                    contentVal.length > 50 ? contentVal
                          .substring(0, 50) + '...': contentVal);
          }
          ...
          fetch(id
            ...
            .catch(err => {
                alert( err.error || 'Error saving post');
              }
            );
        }
      );
    ...
    async function adminDeletePost(
      ...
      try {
        ...
        alert(err.error ||  'Error deleting blog post.');
      }
    }
